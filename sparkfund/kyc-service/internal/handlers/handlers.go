package handlers

import (
	"net/http"

	"github.com/gin-gonic/gin"
	"go.uber.org/zap"

	"sparkfund/kyc-service/internal/models"
	"sparkfund/kyc-service/internal/services"
	"sparkfund/kyc-service/internal/errors"
)

type Handler struct {
	logger  *zap.Logger
	service services.Service
}

func NewHandler(logger *zap.Logger, service services.Service) *Handler {
	return &Handler{
		logger:  logger,
		service: service,
	}
}

func (h *Handler) handleError(c *gin.Context, err error) {
	var appError *errors.Error

	if errors.As(err, &appError) {
		h.logger.Error("Request failed",
			zap.Int("status", appError.Status),
			zap.String("message", appError.Message),
			zap.Error(err))
		c.JSON(appError.Status, models.Error{
			Code:    appError.Status,
			Message: appError.Message,
		})
		return
	}

	h.logger.Error("Request failed",
		zap.Int("status", http.StatusInternalServerError),
		zap.Error(err))
	c.JSON(http.StatusInternalServerError, models.Error{
		Code:    http.StatusInternalServerError,
		Message: "Internal server error",
	})
}

// VerifyIdentity godoc
// @Summary      Verify client identity
// @Description  Verify client's identity using provided documents
// @ID           verifyIdentity 
// @Tags         kyc
// @Accept       json
// @Produce      json
// @Param        request body models.VerificationRequest true "Identity verification request"
// @Success      200 {object} models.VerificationResponse
// @Failure      400 {object} models.Error
// @Failure      500 {object} models.Error
// @Router       /kyc/verify [post]
// @Security     BearerAuth
func (h *Handler) VerifyIdentity(c *gin.Context) {
	var req models.VerificationRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.logger.Error("Invalid request body", zap.Error(err))
		c.JSON(http.StatusBadRequest, models.Error{
			Code:    http.StatusBadRequest,
			Message: "Invalid request format",
		})
		return
	}

	if err := models.ValidateStruct(req); err != nil {
		h.logger.Error("Validation failed", zap.Error(err))
		c.JSON(http.StatusBadRequest, models.Error{
			Code:    http.StatusBadRequest,
			Message: err.Error(),
		})
		return
	}

	response, err := h.service.VerifyIdentity(c.Request.Context(), req)
	if err != nil {
		h.handleError(c, err)
		return
	}

	c.JSON(http.StatusOK, response)
}

// UpdateKYCStatus godoc
// @Summary      Update KYC status
// @Description  Update the KYC verification status for a client
// @ID           updateKycStatus
// @Tags         kyc
// @Accept       json
// @Produce      json
// @Param        request body models.UpdateStatusRequest true "Status update request"
// @Success      200 {object} models.KYCStatus
// @Failure      400 {object} models.Error
// @Failure      404 {object} models.Error
// @Failure      500 {object} models.Error
// @Router       /kyc/status [put]
// @Security     BearerAuth
func (h *Handler) UpdateKYCStatus(c *gin.Context) {
	var req models.UpdateStatusRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.logger.Error("Invalid request body", zap.Error(err))
		c.JSON(http.StatusBadRequest, models.Error{
			Code:    http.StatusBadRequest,
			Message: "Invalid request format",
		})
		return
	}

	if err := models.ValidateStruct(req); err != nil {
		h.logger.Error("Validation failed", zap.Error(err))
		c.JSON(http.StatusBadRequest, models.Error{
			Code:    http.StatusBadRequest,
			Message: err.Error(),
		})
		return
	}

	status, err := h.service.UpdateKYCStatus(c.Request.Context(), req)
	if err != nil {
		h.handleError(c, err)
		return
	}

	c.JSON(http.StatusOK, status)
}